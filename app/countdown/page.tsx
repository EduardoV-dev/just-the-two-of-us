...
function TimeUnit({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="relative bg-surface border border-border rounded-2xl w-18 h-20 md:w-35 md:h-27.5 flex items-center justify-center shadow-sm overflow-hidden">
        <AnimatePresence mode="popLayout">
          <motion.span
            ...
            animate="animate"
            exit="exit"
            className="font-heading text-3xl md:text-5xl text-accent tabular-nums"
          >
            {display}
          </motion.span>
        </AnimatePresence>
...
function Separator() {
  return (
    <div className="flex flex-col items-center justify-center pb-6 mt-8">
      <motion.span
        animate={{ opacity: [1, 0.3, 1] }}
        transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }}
        className="font-heading text-2xl md:text-4xl text-accent-hover"
      >
        :
      </motion.span>
    </div>
...
export default function CountdownPage() {
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center min-h-[80vh] px-6 text-center">
      <motion.div
        initial="hidden"
        animate="visible"
        variants={heroText}
      ...
      >
        ...
        <motion.div
          ...
          className="text-accent text-4xl md:text-5xl mb-6"
        >
          &#10084;
        </motion.div>
